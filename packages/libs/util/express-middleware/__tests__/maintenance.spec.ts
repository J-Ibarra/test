...
import { maintenanceMiddleware } from '../src/maintenance'

describe('maintenance middleware', () => {
  it('request should pass through the middleware and proceed', async () => {
    const request = new MockExpressRequest()

    const successResult = await maintenanceMiddleware(request, null, () => {
      ''
    })
  ...
  it('request should get stopped and error message and 500 status returned', async () => {
    ...
    const successResult = await maintenanceMiddleware(request, mockResponse as any, null)
    const {
      response: { error },
      statusCode,
