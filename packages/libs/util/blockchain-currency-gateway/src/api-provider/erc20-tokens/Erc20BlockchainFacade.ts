import { TransactionResponse, TransactionResponseERC_20, OnChainCurrencyGateway } from '../../currency_gateway'
...
export class Erc20BlockchainFacade implements BlockchainFacade {
  ...
  async getAddressBalance(address: string, contract: string): Promise<number> {
    const addressDetails = await this.cryptoApiProviderProxyEth.getErc20TokenBalance(
      address,
      contract,
    )
  ...
  async subscribeToTransactionConfirmationEvents(transactionHash: string, callbackURL: string): Promise<void> {
    await this.cryptoApiProviderProxyEth.createConfirmedTransactionEventSubscription({ transactionHash, callbackURL, confirmations: this.DEFAULT_ERC20_TRANSACTION_CONFIRMATIONS })
  }

  async subscribeToAddressTransactionEvents(address: string, confirmations: number): Promise<IAddressTransaction> {
    await this.cryptoApiProviderProxyEth.createAddressTokenTransactiontEventSubscription({ address, confirmations, callbackURL: process.env.DEPOSIT_ADDRESS_UNCONFIRMED_TRANSACTION_CALLBACK_URL! })
  }
}
