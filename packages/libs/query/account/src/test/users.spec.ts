...
import { createTemporaryTestingAccount, TEST_PASSWORD } from '../../db/test_helpers/test_accounts'
import { AccountType, EmailValidationError, User } from '../interface'
import {
...
describe('users', () => {
  ...
  describe('activateUser', () => {
    ...
    it('updates 1 field when given correct data', async () => {
      ...
      const [rowsEffected, rows] = await updateUser({ id: account.users[0].id, activated: true } as Partial<User>)
