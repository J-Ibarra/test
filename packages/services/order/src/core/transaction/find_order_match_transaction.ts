...
export async function findLastOrderMatchForSymbols(symbolIds: string[]) {
  return wrapInTransaction(sequelize, null, async (t) => {
    const lastSymbolOrderMatches = await Promise.all(symbolIds.map((symbolId) => findLastOrderMatchForSymbol(symbolId, t)))

    return lastSymbolOrderMatches.filter(Boolean)
  })
}
