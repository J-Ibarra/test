...
import { findWithdrawalRequest } from '../../../core/lib'
import {
  validatePartialMatch,
  stubFindCurrencyForCodesCall,
  stubFindCurrencyBalancesCall,
  USD,
  stubAccountCreation,
} from '../../../core/__test__/utils'
...
describe('Withdrawal Request Initialisation', () => {
  ...
  describe('initialiseWithdrawal', () => {
    it('creates withdrawal in pending state and balance adjustment for given account and crypto currency', async () => {
      ...
      const pushNewCryptoWithdrawalRequestForProcessingStub = sinon
        .stub(withdrawalOperations, 'pushNewCryptoWithdrawalRequestForProcessing')
        .resolves()
      ...
      await initialiseWithdrawal(withdrawalParams)
      ...
      expect(pushNewCryptoWithdrawalRequestForProcessingStub.calledWith(withdrawalRequest!.id)).to.eql(true)
    }).timeout(60_000)
    ...
    it('creates withdrawal in requested state and balance adjustment for given account and fiat currency', async () => {
      ...
      expect(pushNewCryptoWithdrawalRequestForProcessingStub.calledOnce).to.eql(false)
    })
