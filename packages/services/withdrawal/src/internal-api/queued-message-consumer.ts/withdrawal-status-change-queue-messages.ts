...
import { WithdrawalStatusChangeRequestType, AsyncWithdrawalStatusChangeRequest } from '@abx-service-clients/withdrawal/src/async_change_model'
...
async function cancelFiatWithdrawal(request: AsyncWithdrawalStatusChangeRequest) {
  ...
  const withdrawalRequestToCancel = await findWithdrawalRequest({
    adminRequestId: request.payload.adminRequestId,
  })
