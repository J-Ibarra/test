...
export function RegisterRoutes(app: express.Express) {
    ...
    app.post('/api/webhooks/crypto/deposits/address/transactions/unconfirmed',
        function(request: any, response: any) {
            ...
            epicurus.publish(DEPOSIT_ADDRESS_UNCONFIRMED_TRANSACTION_QUEUE_URL, request.body)
    ...
    app.post('/api/webhooks/crypto/deposits/address/transactions/confirmed',
        function(request: any, response: any) {
            ...
            epicurus.publish(DEPOSIT_CONFIRMED_TRANSACTION_QUEUE_URL, request.body)
            response.status(200).end();
        });
    ...
    app.post('/api/webhooks/crypto/deposits/holdings-transactions/confirmations',
        function(request: any, response: any) {
            ...
            epicurus.publish(DEPOSIT_HOLDINGS_TRANSACTION_CONFIRMATION_QUEUE_URL, request.body)
            response.status(200).end();
        });
    ...
    app.post('/api/webhooks/crypto/withdrawals/confirmations',
        function(request: any, response: any) {
            ...
            epicurus.publish(WITHDRAWAL_TRANSACTION_COMPLETION_PENDING_QUEUE_URL, request.body)
            response.status(200).end();
        });
}
