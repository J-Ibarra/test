...
lerna run test --scope $1 --since develop --include-dependencies && lerna run build-image-latest --scope $1 --since develop && lerna run push-image --scope $1 --since develop
